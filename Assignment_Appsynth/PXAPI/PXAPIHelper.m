...
-(NSURLRequest *)urlRequestForCategory:(NSInteger)userID
{
    NSMutableString *urlString = [NSMutableString stringWithFormat:@"%@/users/213451/galleries/11",self.host];
    NSMutableURLRequest *mutableRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlString]];
    [mutableRequest setHTTPMethod:@"GET"];
    
    NSString *accessTokenAuthorizationHeader = OAuthorizationHeader(mutableRequest.URL, @"GET", nil, self.consumerKey, self.consumerSecret, self.authToken, self.authSecret);
    [mutableRequest setValue:accessTokenAuthorizationHeader forHTTPHeaderField:@"Authorization"];
    [mutableRequest setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",urlString]]];
    
    return mutableRequest;
}
-(NSURLRequest *)urlRequestForPhotoPage:(NSInteger)page photoSizes:(PXPhotoModelSize)photoSizesMask  only:(PXPhotoModelCategory)includedCategory {
    
    NSMutableDictionary *options = [@{
                                      //                                      @"feature" : [self stringForPhotoFeature:photoFeature],
                                      //                                      @"rpp" : @(resultsPerPage),
                                      //    @"sort" : [self stringForSortOrder:sortOrder],
                                      @"page" : @(page)} mutableCopy];
...
+(NSString *)nameStringPhotoCategory:(PXPhotoModelCategory)includedCategory{
    
    return [[PXAPIHelper alloc] urlStringPhotoCategoryForPhotoCategory:includedCategory];
